...
5. **References**
   ...
   - [docs/in-process/DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_SNIPPET_CONTRACT.md](DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_SNIPPET_CONTRACT.md)

- [docs/in-process/DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_BUILDER_HANDOFF.md](DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_BUILDER_HANDOFF.md)
...
- [ ] Snippet contract is frozen in [docs/in-process/DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_SNIPPET_CONTRACT.md](DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_SNIPPET_CONTRACT.md), including `user_setup`, `user_validation`, admin setup/validation snippets, `meta_stamp`, and override behavior.
- [ ] Builder handoff brief is frozen in [docs/in-process/DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_BUILDER_HANDOFF.md](DASHBOARD_METADATA_STAMPING_STANDARDIZATION_SUP_BUILDER_HANDOFF.md).
- [ ] Card structure contract is frozen (required card header comment + numbered section order with user/admin configuration first).
- [ ] Validation copy policy is frozen (legacy kidname phrasing removed; consistent neutral wording approved).
- [ ] Admin shared-helper behavior is frozen (which validator snippets are mandatory vs optional, and exact `skip_render` contract).
- [ ] Override policy is frozen (default-off, documented advanced path, no silent bypass of guard logic).
- [ ] Metadata stamp format is frozen (field order, separators, placement rules per card type).
- [ ] File ownership/order is frozen: source edits in `choreops-dashboards/templates/*` first, then sync into ChoreOps vendored templates.
- [ ] Test acceptance criteria are frozen (snippet coverage, header/section contract, override safety, render parse validity).
- [ ] Definition of done is frozen with quality gates and parity checks from this plan.
